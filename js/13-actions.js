...
// ...empty since 0.783: slabs and stairs are no longer blocks at all, but chisel shapes of the full block
...
const _baseStack = (id) => id >= 256 ? (ITEM_PROPS[id]?.stack ?? DEFAULT_STACK) : (PROPS[id]?.stack ?? DEFAULT_STACK);
// Pack Rat (0.79): blocks that stack at all stack 10 higher
const PACK_RAT_BONUS = 10;
const _packRatRoom = (id) => (id < 256 && _baseStack(id) > 1) ? PACK_RAT_BONUS : 0;
const stackSize = (id) => _baseStack(id) + ((typeof hasSkill === 'function' && hasSkill('packRat')) ? _packRatRoom(id) : 0);
/* A tool or piece of gear at 0 durability only exists with the Mender skill: broken, kept, useless
   until repaired (0.79). Every place that asks what the held item DOES asks through heldUseId, so a
   broken pickaxe mines like a bare hand and a broken sword hits like one. */
const slotBroken = (s) => !!s && s.dur === 0;
const heldUseId = () => { const s = HOTBAR[hotbarSel]; return s && !slotBroken(s) ? s.id : null; };
...
/* A slot's per-instance extras beyond id, count and wear (0.79): `fresh` is food's spoilage clock
   (0.789), `wm` marks a tool or piece of gear as Well made (the Fine Work skill). Every path that
   moves a slot through the world — a throw, a drop, a pickup, a chest, a save — carries them as one
   object, so a new field only has to be added to this list. */
const SLOT_META_KEYS = ['fresh', 'wm'];
function slotMeta(s) {
  if (!s) return null;
  let m = null;
  for (const k of SLOT_META_KEYS) if (s[k] != null) (m || (m = {}))[k] = s[k];
  return m;
}
function applyMeta(t, m) {
  if (t && m) for (const k of SLOT_META_KEYS) if (m[k] != null) t[k] = m[k];
  return t;
}
// split/move n items out of an existing slot, KEEPING its wear and its extras
const carrySlot = (src, n) => {
  const t = mkSlot(src.id, n);
  if (src.dur != null) t.dur = src.dur;
  return applyMeta(t, slotMeta(src));
};
...
function _validateSlot(v) {
  ...
  // an old slab or stairs item (before 0.783) comes back as the full block it was made of
  if (typeof v === 'number') v = legacyItemId(v);
  else if (v && typeof v === 'object' && legacyItemId(v.id) !== v.id) v = { ...v, id: legacyItemId(v.id) };
  if (typeof v === 'number' && _isValidId(v)) return mkSlot(v);
  if (v && typeof v === 'object' && _isValidId(v.id)
      && Number.isInteger(v.count) && v.count >= 1 && v.count <= stackSize(v.id) + _packRatRoom(v.id)) {
...
// every fiber roll goes through here: Grass Weaver makes each one 30% likelier (0.79)
const fiberChance = (c) => c * (typeof skillFiberMul === 'function' ? skillFiberMul() : 1);
function _harvestFiber(x, y, z, rolls, chance = HARVEST_FIBER_CHANCE) {
  if (player.canFly) return;
  for (let i = 0; i < rolls; i++)
    if (Math.random() < fiberChance(chance)) _giveFiber(x, y, z);
}
...
/* The variant a chisel shape goes down in, from where it was clicked (0.783):
     stairs          full side toward the player; a ceiling click turns them upside down
     slab            always flat; a ceiling click, or the upper half of a wall, hangs it high
     vertical slab   hugs the clicked wall; on a floor or ceiling it takes the half away from you
     pane            through the middle, its wide face toward you (0.784)
     fence           a post; its rails follow the fences around it, so nothing to choose (0.784) */
function _shapeVariantFor(key, hit, px, py, pz) {
  const ddx = player.pos.x - (px + 0.5), ddz = player.pos.z - (pz + 0.5), alongX = Math.abs(ddx) > Math.abs(ddz);
  if (key === 'carpet') return SHAPE_LAYER;                 // one layer (0.785)
  if (key === 'wall') return SHAPE_WALL;                    // joins its neighbours on its own (0.787)
  if (key === 'cover') {
    /* A cover (0.787) goes where it hides something:
         over a hole (walled in on 3+ sides, open above)  a lid flush with the top of the cell
         in a doorway (walled on both sides along one axis) a plate flush with the wall, on your side —
                                                             click the floor of the gap or its side jamb
         anywhere else                                      flat against the face you clicked */
    const S = (dx, dy, dz) => CORE.solidVal(getBlock(px + dx, py + dy, pz + dz));
    const xp = S(1, 0, 0), xn = S(-1, 0, 0), zp = S(0, 0, 1), zn = S(0, 0, -1);
    const walls = xp + xn + zp + zn;
    if (hit.ny !== -1 && walls >= 3 && !S(0, 1, 0)) return SHAPE_COVER + 1;
    const alongX = xp && xn, alongZ = zp && zn;
    if (hit.ny !== -1 && alongX !== alongZ && (hit.ny === 1 || (alongX ? hit.nx : hit.nz)))
      return alongX ? SHAPE_COVER + (ddz > 0 ? 5 : 4) : SHAPE_COVER + (ddx > 0 ? 3 : 2);
    return SHAPE_COVER + (hit.ny === 1 ? 0 : hit.ny === -1 ? 1 : hit.nx === 1 ? 2 : hit.nx === -1 ? 3 : hit.nz === 1 ? 4 : 5);
  }
  if (key === 'pane') return SHAPE_PANE + (alongX ? 1 : 0);
  if (key === 'fence') return SHAPE_FENCE;
  if (key === 'stairs') return SHAPE_STAIRS + ((alongX ? (ddx > 0 ? 2 : 3) : (ddz > 0 ? 0 : 1)) | (hit.ny === -1 ? 4 : 0));
  if (key === 'slab') {
    const fy = camera.position.y + _dir.y * hit.t - py;
    return SHAPE_SLAB + (hit.ny === 1 ? 0 : hit.ny === -1 ? 1 : fy > 0.5 ? 1 : 0);
  }
  if (hit.ny === 0) return SHAPE_SLAB + (hit.nx === 1 ? 2 : hit.nx === -1 ? 3 : hit.nz === 1 ? 4 : 5);
  return SHAPE_SLAB + (alongX ? (ddx > 0 ? 2 : 3) : (ddz > 0 ? 4 : 5));
}
...
function _doPlace() {
  ...
  /* Carpet layers (0.785): with the chisel's Carpet picked, a stacking block (snow, leaves, sand, gravel,
     fiber) clicked onto a stack that will take it piles on as one more layer — any mix, up to 8. Once the
     stack is full, or will not take it, the click falls through to normal placement (a new layer above). */
  const layerShape = typeof chiselShapedPlace === 'function' ? chiselShapedPlace(heldId) : null;
  if (layerShape && layerShape.key === 'carpet' && canStackLayer(getBlock(hit.x, hit.y, hit.z), heldId)
      && addLayer(hit.x, hit.y, hit.z, heldId)) {
  ...
  // a hollow log is a planter (0.7842): dirt, grass or sand in hand packs into an empty one instead of placing
  if (HOLLOW_FILL_OF[heldId] && isHollowLog(hit.id)) {
    const hv = getBlock(hit.x, hit.y, hit.z);
    if (!hollowFillOf(hv)) {
      setBlock(hit.x, hit.y, hit.z, hv | (HOLLOW_FILL_OF[heldId] << 10));
      playBlockSound(heldId, 'place', hit.x, hit.y, hit.z);
      handPlaceSwing = true;
      spendHeld("placed");
      return;
  ...
  /* the chisel (0.782, variants since 0.783): a shapeable block in hand goes down as the picked slab,
     vertical slab or stairs — same block id, the shape in its variant byte */
  const shaped = typeof chiselShapedPlace === 'function' ? chiselShapedPlace(heldId) : null;
  const halfFill = !!shaped && (shaped.key === 'slab' || shaped.key === 'vslab');
  ...
  // ...or an upright hollow log packed with dirt or grass, the planter (0.7842)
  if (id === B.TALLGRASS || id === B.POPPY || id === B.ORCHID || id === B.TALL_LOWER) {
    const bv = getBlock(px, py - 1, pz);
    if ((bv & 255) !== B.GRASS && !potSoil(bv)) return;
  }
  ...
  if (id === B.CACTUS) {
    ...
    let armAxis = -1;
    for (const [dx, dz] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
      const nv = getBlock(px + dx, py, pz + dz), nid = nv & 255;
  ...
  // don't place a solid inside yourself (a layer you wade through is fine, 0.785)
  if (PROPS[id].solid && !(shaped && ((shaped.key === 'carpet' && PROPS[id].layerStack) || shaped.key === 'cover'))) {
  ...
  if (!shaped && PROPS[id]?.opaque && _plainGrass(getBlock(px, py - 1, pz))) setBlock(px, py - 1, pz, B.DIRT);
  handPlaceSwing = true;
  spendHeld("placed");
  if (shaped) chiselWear();                                // one point of chisel wear per shaped block (0.782)
}
