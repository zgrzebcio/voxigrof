...
const isGrazer = (e) => e.kind === 'sheep' || e.kind === 'cow' || e.kind === 'horse';
...
const SHEEP_SCALE = 0.98, COW_SCALE = 1.15;      // sheep 0.85 -> 0.98, 15% bigger (0.7572)
...
function _mobDropHeld(e) {
  ...
  spawnDrop(e.heldItem, Math.floor(e.x), Math.floor(e.y + 0.5), Math.floor(e.z),
    { x: (Math.random() - 0.5) * 2, y: 2.4, z: (Math.random() - 0.5) * 2 }, 0.6, e.heldDur ?? null);
  e.heldItem = null; e.heldDur = null;
}
...
function tryShearSheep() {
  ...
  if (!player.canFly && slot && slot.dur != null) {
    slot.dur--;
    if (slot.dur <= 0) { HOTBAR[hotbarSel] = null; feedItem(slot.id, -1, 'broke'); }
...
function _updateGrazer(e, dt, pdx, pdz, distXZ, i) {
  ...
  const walkSpeed = horse ? HORSE_WALK_SPEED * horseSpeedStat(e) : cow ? COW_SPEED : SHEEP_SPEED;
  const fleeSpeed = horse ? HORSE_FLEE_SPEED * horseSpeedStat(e)
                          : cow ? COW_FLEE_SPEED : SHEEP_FLEE_SPEED;
...
function updateEntities(dt) {
  ...
  for (let i = ENTITIES.length - 1; i >= 0; i--) {
    ...
    const huntingInWater = inWater && mindless && e.state === 'chase' && !tp.dead;
    if (inWater && !huntingInWater) {
      ...
      e.thinkT -= dt;
      moveSpeed = 0;
    } else if (e.kind === 'skeleton' && e.arm === 'bow' && e.ammo > 0 && e.state === 'chase' && !tp.dead) {
      ...
      const aimY = tp.pos.y + (tp.EYE || 1.62) * 0.6;
      const los = hasLineOfSight(e.x, e.y + 1.45, e.z, tp.pos.x, aimY, tp.pos.z);
      e.faceYaw = Math.atan2(pdx, pdz);
      if (e.shootCd > 0) e.shootCd -= dt;
      if (distXZ > SKEL_SHOOT_RANGE || !los) {
        e.yaw = e.faceYaw; moveSpeed = SKEL_CHASE_SPEED; e.drawT = 0;
      } else if (distXZ < SKEL_KEEP_DIST) {
        ...
        if (distXZ < entAttackRange(e, tp) && Math.abs(pdy) < 2 && e.atkCd <= 0) {
        ...
        if (e.shootCd <= 0) {
          ...
          if (e.drawT >= SKEL_DRAW_TIME) {
            ...
            e.ammo--;
            mobShoot(e, tp, { ammoId: e.ammoId, dmg: SKEL_ARROW_DMG, speed: SKEL_ARROW_SPEED });
