...
function _activeShield() {
  ...
  if (main && isShieldId(main.id)) return { slot: main, hand: 'main' };
  const off = offhandShield();
  if (off && !mainHandUsesRightClick(slotId(main))) return { slot: off, hand: 'off' };
  return null;
}
...
function shieldBlock(tp, fromX, fromZ, dmg) {
  ...
  withPlayer(tp, () => {
    ...
    a.slot.dur -= Math.max(1, Math.ceil(dmg || 1));
    if (a.slot.dur <= 0) {
      if (a.hand === 'main') HOTBAR[hotbarSel] = null;
      else equipSlots[EQUIP_INDEX.offhand] = null;
      tp.blocking = false;
      feedItem(a.slot.id, -1, 'broke');
      playSound('toolBreak', { gain: 0.8 });
    }
