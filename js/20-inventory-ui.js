...
function itemTooltipHTML(id, dur) {
  ...
  if (p.equip) {
    ...
    conds.push(['worn', p.packSlots ? `+${p.packSlots} backpack slots`
                      : p.beltSlots ? `+${p.beltSlots} belt slots` : 'No effect',
...
function itemDisplayName(id, dur) {
  ...
  if (!p) return '';
  const max = (p.tool || p.ranged || p.shield) ? p.durability : 0;
...
function dropHovered(n = 1) {
  ...
  throwFromPlayer(s.id, k, s.dur ?? null);
